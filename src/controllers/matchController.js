...
const findMatchingDonors = async (req, res) => {
  try {
    ...
    if (request.requester.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Not authorized to find matches for this request" });
    }
    ...
    if (request.hospital && request.hospital.city) {
      ...
      matchedDonors = donors.filter(
        (donor) =>
          donor.location &&
          donor.location.city &&
          donor.location.city.toLowerCase() ===
            request.hospital.city.toLowerCase()
      );
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to find matching donors" });
  }
};
...
const volunteerForRequest = async (req, res) => {
  try {
    ...
    if (!["pending", "in-progress"].includes(request.status)) {
      return res
        .status(400)
        .json({ message: "This request is no longer accepting donors" });
    }

    // Check if user is eligible to donate
    const user = req.user;
    const isEligible =
      typeof user.isEligibleToDonate === "function"
    ...
    if (!isEligible) {
      return res.status(400).json({
        message:
          "You are not eligible to donate at this time. Please wait at least 3 months between donations.",
      });
    }
    ...
    res
      .status(400)
      .json({ message: error.message || "Failed to volunteer for request" });
  }
};
