...
const createDonation = async (req, res) => {
  try {
    const { requestId, hospital, donationDate, units, notes } = req.body;
    ...
    res
      .status(400)
      .json({ message: error.message || "Failed to record donation" });
  }
};
...
const getUserDonations = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to get donations" });
  }
};
...
const getDonationById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to get donation" });
  }
};
...
const verifyDonation = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to verify donation" });
  }
};
